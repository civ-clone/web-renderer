...
| 1 `#private` → `private` | **Converted, not published** | All 62 packages committed on `master` in their own checkouts. Engine state checksums unchanged; the one wire-format change is recorded in `05-engine-plan.md`. Nothing pushed and nothing published. |
| 2–7 | Not started | |
