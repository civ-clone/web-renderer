...
- **Status: partly fixed 2026-09-22** — the six scenery layers (Land,
  ...
  Viewport-sized buffers are still open (#7) but no longer blocked: the
  minimap draws from an `Overview` layer of its own (#53) rather than scaling
  down `Landscape`, `Cities` and `ActiveUnit`, so nothing outside `Portal`
  reads a layer's `canvas()` any more. That also removed what had become the
